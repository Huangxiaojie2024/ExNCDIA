...
# Molecule drawing is optional. RDKit's Draw module needs system X11
# libraries (provided via packages.txt). If it is unavailable for any
# reason, the app still runs and shows predictions.
try:
...
if st.button("Predict", type="primary"):
    ...
    # Structure image (optional - never let it crash the app)
    if DRAW_OK:
        try:
            st.image(Draw.MolToImage(mol, size=(300, 240)),
                     caption="Standardized structure")
        except Exception:
            pass
